var _t_ab = function(v) { return typeof translate_ability === "function" ? translate_ability(v) : v; };
var _t_it = function(v) { return typeof translate_item === "function" ? translate_item(v) : v; };
...
function getKOChanceText(damageIn, move, defender, field, isBadDreams) {
    ...
    if (field.weather === 'Sun') {
        if (defender.ability === 'Dry Skin' || defender.ability === 'Solar Power') {
            ...
            eotText.push(_t_ab(defender.ability) + '的扣血');
        }
    } else if (field.weather === 'Rain') {
        if (defender.ability === 'Dry Skin') {
            eot += Math.floor(Math.floor(defender.maxHP / 8) * maxChip);
            eotText.push('干燥皮肤的回复');
    ...
    if (c === 1) {
        ...
        return '有 ' + percNumText + '% 的机率一击KO' + afterText;
    }

    if (restoreHP && move.name !== 'Knock Off' && !(defender.item.includes(' Berry') && ['Bug Bite', 'Pluck', 'Incinerate'].includes(move.name))) {
        let eotTemp = '';
        if (isRipen) eotTemp += '熟成的';
        else if (isGluttony) eotTemp += '贪吃鬼触发的';
        eotTemp += _t_it(defender.item) + '回复';
    ...
    afterText = hazardText.length > 0 || eotText.length > 0 ? ' (在计算' + serializeText(hazardText.concat(eotText)) + '后)' : '';
    if (c === 1) {
        return '确定一击KO' + afterText;
    } else if (c > 0) {
        if (c < 0.0001)
    ...
    for (i = 2; i <= 4; i++) {
        ...
        if (c === 1) {
            ...
            if (c < 0.0001)
                ...
                percNumText = Math.round(c * 10000) / 100;
            return '有 ' + percNumText + '% 的机率 ' + i + '次攻击击杀' + afterText;
        }
    }
...
function serializeText(arr) {
    if (arr.length === 0) {
        ...
        return text + '以及 ' + arr[arr.length - 1];
    }
}
